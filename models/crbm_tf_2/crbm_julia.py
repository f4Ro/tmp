...
class CRBM(object):
    ...
    def do_contrastive_divergence(self: CRBM, data, n=1, global_step=0):
        ...
        if self.padding:
            ...
            eingabe=tf.transpose(V0, perm=[3, 1, 2, 0]),  # 1, 120, 1, 1
            filt= tf.transpose(Q0, perm=[1, 2, 0, 3]),  # 120, 1, 1, 24
            positive = tf.nn.conv2d(
        ...
        ret_w = self._apply_grad(self.kernels, g_weight, self.vitesse_kernels, wd=True, wd_value=g_weight_l2, sparsity=True,
